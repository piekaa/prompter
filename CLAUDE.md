...
**Anti-jitter + catch-up.** Position only moves forward (monotonic). A candidate commits only if it beats the current position's score by the base margin (forwardMargin·W = 0.15·W) **and** is either a **strong** match (≥ `strongMatchMin` = 4 real exact/diacritic words in the window — may sit up to `searchRadius` = 80 ahead: this is the catch-up that fires when the speaker reads ahead or ASR misses a chunk) or **near** (within `nearRadius` = 12). Weak matches (a few words) are therefore distance-gated, so 1–2 coincidental words can't commit a large jump to a repeated/similar phrase further in the text (the old flat margin let 1–2 spoken words highlight ~20 words; a distance-scaled margin over-corrected and blocked legitimate catch-up — the fix is strength-based, not distance-based). `preview()` uses the same rule — don't let it become evidence-free again. Among committable candidates the highest score wins; ties keep the nearest occurrence. Similarity tiers: exact = 1.0, diacritic-fold match = 0.5 (these count as "real" matches), fuzzy (Levenshtein) capped at 0.3 (adds score but doesn't count as real), ratio ≥ 0.5 → 0. Don't lower `strongMatchMin` below 4 or raise the fuzzy cap casually — that's what keeps `p` from jittering on a ~12–18% WER Polish model.
